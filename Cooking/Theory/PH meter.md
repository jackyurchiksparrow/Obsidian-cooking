...
#sourdough #panettone #ph_meter

A device that is able to measure acidity of liquid or (depending on the device) semi-solid entities (such as dough and other foods). 
Extremely useful for [[Lievito madre]] where the perfect value after the very last feeding between panettone is **4.1**.

## Before first use
1. Soak in cleaning solution for an hour (just in case).
2. Rinse with distilled water by inserting the electrode in and stirring. Discard the water.
3. Soak in storage solution for an hour

## (Re)calibrate when:

- first use
- stored in noisy environments where vibrations affect it frequently
- used every day -> recalibrate every 7-10 days
- the batteries are changed
- the readings are inconsistent / clearly off
- electrode wasn't stored properly and/or dried out
- was stored correctly but unused for 2 weeks+
